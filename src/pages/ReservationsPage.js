...
const QRCodeModal = ({ gates, selectedGate, setSelectedGate, reservationId, cost, isCheckout, onClose, onCheckin, onPayment, onCheckout }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-xl shadow-lg text-center w-[500px] relative">
        ...
        <button
          onClick={() => {
            if (cost > 0 && !isCheckout) {
              ...
              onCheckout(selectedGate, reservationId);
            } else {
              onCheckin(selectedGate, reservationId);
            }
          }}
        >
          <img
        ...
        <p className="text-sm text-gray-600 mb-4">Reservation Number <br /> {reservationId}</p>
        ...
        {cost <= 0 && (
          <div className="mb-4">
            <select
              id="gate"
              name="gate"
              className="block w-full border border-gray-300 rounded-lg px-4 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-center"
              value={selectedGate}
                ...
                <>
                  ...
                  {gates.map((gate) => (
                    <option key={gate.ID} value={gate.ID}>
                      {gate.GateEntrance} - {gate.ParkingLotName} ({gate.Status === 'closed' ? 'Available' : 'Unavailable'})
                    </option>
                  ))}
                </>
              )}
            ...
            {gates?.length === 0 && (
              <p className="mt-2 text-sm text-red-500 italic">Please check gate availability.</p>
            )}
          </div>
        )}
    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-transform hover:-translate-y-1 p-6">
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          {canCheckIn && (
            <button
              onClick={() => onQrCode(reservation.parking_lot_id, reservation.id)}
          ...
          {reservation.status === 'active' && reservation.payment_status === 'paid' ? (
            <button
              onClick={() => onQrCode(reservation.parking_lot_id, reservation.id, 0, 'checkout')}
...
const ReservationsPage = () => {
  ...
  const handleQrCode = async (parkingLotId, reservationId, cost = 0, type = null) => {
    if (parkingLotId != "") {
      await loadGateByParkingLotId(parkingLotId);
    }
  ...
  const handleCheckin = async (gateId, reservationId) => {
    try {
      if (!gateId) {
      ...
      await checkinReservation(gateId, reservationId);
      setSelectedGate('');
      closeModal();
    } catch (error) {
      // Error handling is managed by the context
  ...
  const handleCheckout = async (gateId, reservationId) => {
    try {
      if (!gateId) {
        toast.error('Please select a gate');
        return;
      ...
      await checkoutReservation(gateId, reservationId);
      setSelectedGate('');
      closeModal();
    } catch (error) {
      // Error handling is managed by the context
