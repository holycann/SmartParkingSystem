...
export const ReservationProvider = ({ children }) => {
  ...
  const loadGateByParkingLotId = useCallback(async (parkingLotId) => {
    setLoading(true);
    try {
      const data = await fetchGateByParkingLotId(parkingLotId);
  ...
  const checkinReservation = async (gateId, reservationId) => {
    setLoading(true);
    try {
      const response = await apiCheckinReservation(gateId, reservationId);
  ...
  const checkoutReservation = async (gateId, reservationId) => {
    setLoading(true);
    try {
      const response = await apiCheckoutReservation(gateId, reservationId);
